...
func (mpc *MPC) RandomShare() *pbc.Element {
	...
	shares := make(chan *pbc.Element, len(mpc.Parties))

	for _, party := range mpc.Parties {
		go func(party *Party) {
			shares <- party.getRandomShare()
		}(party)

	}

	share := zero
	for _ = range mpc.Parties {
		nextShare := <-shares
		share = mpc.Pk.EAddElements(share, nextShare)
	}
...
func (mpc *MPC) solvedBits() ([]*pbc.Element, *pbc.Element, error) {
	...
	bits[n-1] = mpc.Pk.EncryptDeterministic(big.NewInt(0))
...
func (mpc *MPC) EBitsPrefixOR(bits []*pbc.Element) []*pbc.Element {
	...
	res := make([]*pbc.Element, lambda*lambda)
	for i := 0; i < lambda; i++ {
		go func(i int) {
			for j := 0; j < lambda; j++ {
				...
				res[i*lambda+j] = mpc.ReEncryptElementMPC(sum)
			}
			wait <- i
		}(i)
	}
...
func (mpc *MPC) BitsBigEndian(a *big.Int, n int) []*big.Int {
	...
	//fmt.Println("[DEBUG]: BitsBigEndian()")
